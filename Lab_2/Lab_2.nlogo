  ...
  ask walkers [
    ifelse patch-here = goal [
      ifelse tofrom = 1[
        ...
        set goal one-of patches in-radius flying-radius
        set tofrom abs(tofrom - 1)
      ]
    ][
      walk-towards-goal
...
to set-new-goal
  ifelse count hubs-here = 1 [
    set goal one-of patches in-radius flying-radius
  ][
    set goal min-one-of (hubs)[distance myself]
  ]
end
...
to recolor-popular-paths
  ask patches with[popular-patch = 1] [set pcolor black]
end

to check-hub-placement
  ifelse mouse-down? [
    if not mouse-clicked? [
      set mouse-clicked? true
      ask patch mouse-xcor mouse-ycor [ toggle-hub ]
    ]
  ] [
...
to toggle-hub
  let nearby-hubs hubs in-radius 4
  ifelse any? nearby-hubs [
